...
def parse_commandline():
    ...
    parser = optparse.OptionParser(usage=__doc__,\
        version="$Id: NEDcompilerPA7,v 7.0 10/04/2017")
    ...
    if opts.filename is None and opts.outname is None:
        print
        print "NEDcompilerPA7.py  version: 7.0   10/04/2017"
        print 
        print "A program to grab galaxy data from the NED server by means of ascii table"
        print "distributions and html parsing, and recompile specified data into a new csv file(s)"
        print
        print "Normal usage example: python NEDcompilerPA7.py -f names.txt -n outFile -o /Users/me/ -v"
        print
    ...
    def run(self):
        while True:
            ...
            if host[1] ==1:
                ...
                # if it's just an ascii_bar table
                warnings.simplefilter("ignore")
                reader = 'x'
                try:
                    reader = ascii.read(url, data_start=9, delimiter='|')
                except Exception, e:
                    sys.stderr.write("\n Unable to read ascii_bar url. Here is the message built into the exception: \n %s \n" %e)

                self.outQueue.put(reader)
                warnings.resetwarnings()
...
class galaxyClass(object):
    ...
    def __init__(self,name,votable,reader,opts):
        self.name = name
    ...
    def returnDiametersandPA_ascii(self,opts):
        # find and return major and minor diameters
...
#         hosts = [\
#         ["http://ned.ipac.caltech.edu/cgi-bin/datasearch?objname={0}&search_type=Diameters&of=xml_main".format(name),1],\
#         ["http://ned.ipac.caltech.edu/cgi-bin/datasearch?objname={0}&search_type=Diameters&of=ascii_bar".format(name),2]]
        
        hosts = [["http://ned.ipac.caltech.edu/cgi-bin/datasearch?objname={0}&search_type=Diameters&of=ascii_bar".format(name),2]]
        ...
        galaxy = galaxyClass(name,votable,votable,opts)
#       galaxy.queryNED()
        sys.stdout.write("1...\n")
        sys.stdout.flush()
...
#         pa = diametersandPA[3]
        ...
        diametersandPA,completeList = galaxy.returnDiametersandPA_ascii(opts)
        freqTargeted = diametersandPA[0]
        diameters = diametersandPA[1]
        dRatio = diametersandPA[2]
        pa = diametersandPA[3]
