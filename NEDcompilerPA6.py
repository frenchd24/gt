...
v6: update to include errors for all returned values (5/17/17)
    --Apparently errors are not there, so don't use this version
...
def parse_commandline():
    ...
    parser = optparse.OptionParser(usage=__doc__,\
        version="$Id: NEDcompilerPA6,v 6.0 05/17/2017")
    ...
    if opts.filename is None and opts.outname is None:
        print
        print "NEDcompilerPA6.py  version: 6.0   05/17/2017"
        print 
        ...
        print
        print "Normal usage example: python NEDcompilerPA6.py -f names.txt -n outFile -o /Users/me/ -v"
        print
...
class galaxyClass(object):
    ...
    def returnDiametersandPA(self,opts):
        ...
        # chosen = [freq, (maj,min),(majErr,minErr), dRatio, dRatioErr, pa, paErr]
        chosen = ['x',('x','x'),('x','x'),'x','x','x','x']
        completeList = ['x']
        try:
            warnings.simplefilter("ignore")
            ...
            majorErr = table.array['d_col20'].tolist()

            minorD = table.array['d_col26'].tolist()
            minorErr = table.array['d_col28'].tolist()
            
            dRatio = table.array['d_col21'].tolist()
            dRatioErr = table.array['d_col30'].tolist()

            pa = table.array['d_col25'].tolist()
            paErr = table.array['d_col36'].tolist()
            
            # make complete list
            completeList = []
            for i in range(len(freqTargeted)):
                completeList.append([freqTargeted[i],(majorD[i],minorD[i]),(majorErr[i],minorErr[i]),dRatio[i],dRatioErr[i],pa[i],paErr[i]])
            ...
            print
            chosen = [freqTargeted[m],(majorD[m],minorD[m]),(majorErr[m],minorErr[m]),dRatio[m],dRatioErr[m],pa[m],paErr[m]]
            warnings.resetwarnings()
...
#       galaxy.queryNED()
        ...
        diameterErrs = diametersandPA[2]
        dRatio = diametersandPA[3]
        dRatioErr = diametersandPA[4]
        pa = diametersandPA[5]
        paErr = diametersandPA[6]
        ...
        if not isNumber(diameterErrs[0]) and not bfind(diameterErrs,'x'):
            diameterErrs = ('x','x')
        if not isNumber(dRatioErr) and not bfind(dRatioErr,'x'):
            dRatioErr = 'x'
        if not isNumber(paErr) and not bfind(paErr,'x'):
            paErr = 'x'
        ...
        objectInfoList = [oldName.replace(' ',''),freqTargeted,diameters,diameterErrs,dRatio,dRatioErr,pa,paErr,completeList]
